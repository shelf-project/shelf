...
exactly the shape we want DuckDB to walk on the cold run (one footer
...
def build_events_table(n_rows: int, n_days: int) -> pa.Table:
    ...
    # Mark each field non-nullable so PyIceberg's schema-compatibility
    # check (Iceberg `required` vs Arrow nullability) succeeds on append.
    schema = pa.schema(
        [
