...
calendar days, identity-partitioned on `event_date`. PyIceberg's writer
fans the rows into one Parquet data file per partition value, which is
exactly the shape we want StarRocks to walk on the cold run (one footer
+ one row group per file × 30 files).

Deterministic via random.seed(12).

This file is a near-verbatim copy of the sister DuckDB example's
seeder (examples/duckdb/init/seed_iceberg.py). Kept duplicated rather
than shared so each example stays self-contained — the user task
explicitly forbids touching anything outside examples/starrocks/.
"""
...
def main() -> int:
    ...
    # Mark all fields nullable (`required=False`) to match PyArrow's
    # default-nullable columns produced by `pa.array(...)` above.
    # Otherwise pyiceberg raises a "Mismatch in fields: required vs
    # optional" check error at append() time.
