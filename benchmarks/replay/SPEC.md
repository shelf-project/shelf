...
_Replays 7 days of `your_query_log_table` from rep-2 against
...
- **Trace source.** `your_query_log_table` filtered on
...
- **Not a correctness test.** Query correctness is assumed; if a
  ...
  tests.
- **Not synthetic.** Trace comes from `your_query_log_table`.
