...
_Deciders: eng-lead, trino-plugin-eng-1, rust-engineer-1_
_Supersedes: none_
_Superseded-by: none_
_Related: SHELF-10, SHELF-12, SHELF-22_
...
During SHELF-22 we verified against the Trino 480 JARs (`javap
io.trino.spi.Plugin`) and against `trinodb/trino@master` that
...
_not_ a plugin extension point today. Trino's own `TrinoFileSystemCache`
...
- **The Java `ShelfFileSystemFactory` is currently dormant but not
  ...
  is actively used by `ShelfPrefetchListener` (which _does_ wire through
