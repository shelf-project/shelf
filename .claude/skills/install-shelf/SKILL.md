...
If the sha256 matches what `aws s3api get-object --bucket $BUCKET
--key $KEY /dev/stdout | sha256sum` returns, the shim is healthy.
...
- **Don't pre-warm during peak traffic** — pre-warm runs `alluxio fs
  load --submit` (or the Shelf equivalent) and competes with live
