...
> 30-day your_query_log_table trace."
...
| Column        | Type    | Meaning |
|---|---|---|
| `timestamp_ms` | bigint  | IST-converted millisecond timestamp. `query_date` in `your_query_log_table` is UTC; the SQL converts to IST. |
...
- `your_query_log_table.inputs_json` records one entry per
...
1. **Granularity is `(query, table)`**, not file or row group. Per
   ...
   granularity `your_query_log_table` honestly supports
