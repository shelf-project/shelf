...
#
# Reads query history from `your_query_log_table` (or whatever source
...
#
# Uploads to s3://example-cdp-temp/shelf/pin_list.json (configurable).
...
#   `your_query_log_table` over the last 7 days, ranked by
...
TOP_5_PROD_TABLES: list[tuple[str, str, str]] = [
    ("warehouse", "your_schema", "your_users_table"),
    ("cdp", "lsq_pw", "silver_prospect_activity_extension_base"),
    ("warehouse", "your_schema", "your_transactions_table"),
    ("cdp", "mview", "gold_dbt_video_stats_v3"),
    ("warehouse", "your_schema", "your_orders_table"),
]
...
def top_n_tables(
    ...
    Schema notes (verified 2026-04-27 against `your_query_log_table`):
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default="s3://example-cdp-temp/shelf/pin_list.json",
