...
- `your_query_log_table` is a goldmine: every query's SQL, plan,
...
t=0.01s  ShelfPrefetchListener receives:
           tables      = [your_catalog.your_schema.your_events_table]
...
EXPLAIN (FORMAT JSON)
SELECT * FROM your_catalog.your_schema.your_events_table
...
nightly by the trainer from `your_query_log_table` — top-N
...
1. The trainer analyses `your_query_log_table` for
...
- Manual pin: `shelfctl pin your_catalog.your_schema.your_events_table`.
...
4. **Real replay**: replay last 7 days of `your_query_log_table`
