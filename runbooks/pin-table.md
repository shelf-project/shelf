...
echo "SELECT sum(scanned_bytes) FROM your_query_log_table WHERE query LIKE '%silver_offline_event%' AND create_time > now() - INTERVAL '7' DAY" \
  | trino --catalog cdp
```
...
1. **Propose via PR** against `shelf-config` repo's `pin_list.json`.
   ...
   ```json
   {
     "pins": [
       {
         "table": "your_catalog.your_schema.your_events_table",
