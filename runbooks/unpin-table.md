...
kubectl -n shelf exec shelf-0 -- shelfctl stats --pin your_catalog.your_schema.your_events_table

# 3. Is the table still receiving queries? Avoid an unpin-then-repin cycle.
echo "SELECT count(*) FROM your_query_log_table WHERE query LIKE '%silver_offline_event%' AND create_time > now() - INTERVAL '24' HOUR" \
  | trino --catalog cdp
```
