...
|---|---|---|
| trino-replica-0 | `shelf-0.shelf.alluxio.svc:9092` (currently reverted to S3) | `shelf-pool.alluxio.svc:9092` |
| trino-replica-1 | `shelf-1.shelf.alluxio.svc:9092` (currently reverted-in-flight) | `shelf-pool.alluxio.svc:9092` |
| trino-replica-2 | `shelf-2.shelf.alluxio.svc:9092` (live) | `shelf-pool.alluxio.svc:9092` |
| trino-replica-3 | direct S3 (never cut over) | `shelf-pool.alluxio.svc:9092` |
...
4. Verify DNS: `kubectl run -n alluxio --rm -it dnsutil --image=infoblox/dnstools -- dig shelf-pool.alluxio.svc.cluster.local`
5. End-to-end probe: `curl -X HEAD http://shelf-pool.alluxio.svc.cluster.local:9092/<bucket>/<key>` (should hit any of the 3 pods, response is identical because shelfd routes internally)
...
gathered from `cdp.trino_logs.trino_queries` per replica before we
