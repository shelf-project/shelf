...
pub async fn handle_get_object(
    ...
    // SHELF-23 — race the HRW primary peer against origin on a local
    // cache miss. `peer_or_origin_fetch` short-circuits to the bare
    // origin future when the local pod is the HRW owner, when the
    // ring is empty (startup), or when peer-fetch has been disabled
    // at runtime — so the pre-SHELF-23 hot path is preserved bit-for-
    // bit on those paths.
    let state_for_peer = state.clone();
    let key_for_peer = key_obj.clone();
    let fetcher = async move {
        peer_or_origin_fetch(
            &state_for_peer,
            pool,
            &key_for_peer,
            offset,
...
mod tests {
    ...
    fn table_label_parses_iceberg_data_path() {
        ...
        let lbl = table_label("analytics/events/data/00000-0-abc.parquet");
        assert_eq!(lbl.as_ref(), "analytics.events");
    }

    #[test]
    fn table_label_parses_iceberg_metadata_path() {
        let lbl = table_label("analytics/page_views/metadata/00001-uuid.metadata.json");
        assert_eq!(lbl.as_ref(), "analytics.page_views");
    }
    ...
    fn table_label_handles_leading_slash() {
        ...
        let lbl = table_label("/analytics/events/data/x.parquet");
        assert_eq!(lbl.as_ref(), "analytics.events");
    }
