...
It originated on an internal example Trino-on-EKS footprint, which
is how the rollout-v1 production evidence exists — but the project
is not a example product, does not ship under a example domain,
and its governance does not report to example. The internal
...
| -------- | ------------- | --- |
| **GitHub org** | **`github.com/shelf-project/shelf`** — new neutral org, stood up before T-0 | Project is independent of the org it originated in. Keeping the repo on a company-owned org would read as a vendor project and muddy governance. One-time org-standup cost (~1 day) beats perpetual "is this a example thing?" question on every thread. |
...
Problem: `example` appears in ~30 files. Two classes:
...
| ---- | --------- | ------------ |
| `Cargo.toml` | `repository = "https://github.com/example-oss/shelf"` | `https://github.com/shelf-project/shelf` |
...
| `charts/shelf/values.yaml` | `image.repository: ghcr.io/example-oss/shelf/shelfd` | `ghcr.io/shelf-project/shelfd` |
| `charts/shelf/ci/lint-values.yaml` | same as values.yaml | same substitution |
| `clients/trino/pom.xml` | `<url>https://github.com/example-oss/shelf</url>` | `https://github.com/shelf-project/shelf` |
...
Airflow / runbook hosts on `*.example.internal`. Two sub-choices:
...
- `docs/rollout-v1.md` + `docs/rollout-v1/` — "we ran this on a
  example Trino-on-EKS cluster" is the rollout narrative; it is
...
4. [ ] `shelf-project.dev` domain registered; `conduct@`,
       ...
       24h ack SLAs.
5. [ ] §1.1 Class A sweep: every `example-oss` / `example.internal` /
