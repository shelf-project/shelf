...
```
      ...
      │  Reads your_query_log_schema.*        │ │  1.5) — HMS poll every 30s│
...
`your_query_log_table` nightly, produces `pin_list.json`,
...
| T-T1 | T | Training data (`your_query_log_schema`) is polluted by a malicious user's crafted queries to skew pins toward their own tables | Trainer filters queries where `state != 'FINISHED'` and `query_type in ('SELECT', 'INSERT')`; drops the top 0.1 % by scanned_bytes as outliers; produces a PR-shaped diff (not direct write) — ops reviews the delta before publication (ADR-0001) | mitigated  |
...
| T-D1 | D | A misbehaving query floods `your_query_log_table` so the nightly job times out | Trainer runs with a 2-hour SLA; if exceeded, it falls back to the previous `pin_list.json` (no-op); PagerDuty warn, not page | mitigated  |
